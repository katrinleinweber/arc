for (p in poles){
  ...
  load(file=file.path(sprintf("prep/spatial/seaice/%s/%s_rasters_points.rdata", assessYear, p)))
  ...
  write.csv(z.h.T, sprintf('prep/CP/int/%s_IceEdgeHabitat_ref%sto%s.csv', p, min(ref.years), max(ref.years)), row.names=FALSE)
  write.csv(z.p.T, sprintf('prep/CP/int/%s_IceShoreProtection_ref%sto%s.csv', p, min(ref.years), max(ref.years)), row.names=FALSE)
